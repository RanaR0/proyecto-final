...
class Receta_Ingrediente(db.Model):
    ...
    id_ingrediente = db.Column(
        db.Integer, db.ForeignKey("INGREDIENTES.id", ondelete="CASCADE"), nullable=False
    )
    ...
    receta = db.relationship(
        "Receta",
        backref=db.backref("ingredientes_asociados", cascade="all, delete-orphan"),
    )

    ingrediente = db.relationship(
        "Ingrediente",
        backref=db.backref("recetas_asociadas", cascade="all, delete-orphan"),
    )

    def __init__(self, id_receta, id_ingrediente, cantidad, unidad):
