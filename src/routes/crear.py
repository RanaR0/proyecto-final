...
def validate_form_data(nombre, descripcion, tiempo, raciones, categoria):
    ...
    if len(nombre.strip()) > MAX_TITLE_LENGTH:
        return False, (f"El título no puede exceder {MAX_TITLE_LENGTH} caracteres.")
    ...
    if len(descripcion.strip()) > MAX_DESCRIPTION_LENGTH:
        return False, (
            f"La descripción no puede exceder " f"{MAX_DESCRIPTION_LENGTH} caracteres."
        )
...
def _safe_next_url(raw_next, fallback_url):
    """Permite solo rutas internas para evitar open redirect."""
    if not raw_next:
        return fallback_url
    ...
    # URL absoluta del mismo host -> convertir a ruta interna
...
def actualizar():
    ...
    """
    next_url = url_for("cuenta.perfil")
    try:
        usuario = obtener_usuario_actual()
        if not usuario:
