...
_SESSION_KEY = "form_errors"

# Blueprint de la sección "cuenta"
cuenta_bp = Blueprint("cuenta", __name__)


def _safe_back_url(raw_next, fallback_url):
    """
    Valida una URL de retorno para evitar redirecciones externas.

    Solo permite:
    - Rutas internas relativas.
    - URLs absolutas pertenecientes al mismo host.

    Args:
        raw_next (str): URL recibida desde parámetros o formularios.
        fallback_url (str): URL segura alternativa.

    Returns:
        str: URL interna segura para la redirección.
    """
    if not raw_next:
        return fallback_url
...
def index():
    ...
    autores_map = {
        u.id: u for u in Usuario.query.filter(Usuario.id.in_(ids_propietarios)).all()
    }

    recetas_data = []
    for r in recetas:
        autor = autores_map.get(r.id_propietario)
        nombre_autor = f"{autor.usuario}" if autor else "Desconocido"
        recetas_data.append(
            {
                "id": r.id,
                "nombre": r.nombre,
...
def ver_receta(id):
    ...
    back_url = _safe_back_url(
        ...
        url_for("cuenta.index"),
    )
    return render_template(
        "cuenta/receta.html", receta=receta, autor=autor, back_url=back_url
    )
...
def set_form_errors(
    ...
    """Guarda errores para `form_id` en la sesión.

    Args:
        form_id: identificador del formulario (ver migration-matrix.md).
        fields: errores por campo (clave = name del input).
        values: valores no sensibles para repintar (sin contraseñas).
        global_error: error general no asociado a campo concreto.
    """
    bucket = _bucket()
    bucket[form_id] = {
...
def _bucket() -> dict:
    """
    Obtiene o crea el contenedor de datos temporales en sesión.

    Se utiliza para almacenar información temporal relacionada
    con formularios y validaciones.

    Returns:
        dict: Diccionario almacenado en la sesión.
    """
...
def actualizar_perfil():
    ...
    Returns:
        Response:
            ...
            - Redirección al formulario con errores si la validación falla.
            - Redirección a ajustes si la actualización es exitosa.
    """
    usuario = obtener_usuario_actual()
    if not usuario:
        return redirect(url_for("auth.login"))
    ...
    if not apellidos:
        flash("Los apellidos son obligatorios.", "error")
        return redirect(url_for("cuenta.editar_perfil"))
    ...
    if contrasena_nueva:
        if not contrasena_actual:
            flash("Introduce tu contraseña actual.", "error")
            return redirect(url_for("cuenta.editar_perfil"))
        ...
        if not contrasena_confirmar:
            flash("Confirma la nueva contraseña.", "error")
            return redirect(url_for("cuenta.editar_perfil"))
    ...
    if fields:
        set_form_errors("perfil_edit", fields=fields, values=echo)
        return redirect(url_for("cuenta.editar_perfil"))
